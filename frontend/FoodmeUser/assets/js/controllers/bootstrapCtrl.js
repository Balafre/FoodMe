...
app.controller('ModalInstanceCtrl', ["$window", "$document", "$scope", "$rootScope", "$cookies", "$timeout", "Facebook", "newHttpPostQuery", "$httpParamSerializer", "$uibModalInstance", "modalWindow", 'dataSer', 'AuthService', function ($window, $document, $scope, $rootScope, $cookies, $timeout, Facebook, newHttpPostQuery, $httpParamSerializer, $uibModalInstance, modalWindow, dataSer, AuthService) {
        ...
        if (form.$invalid) {
            ...
            if($scope.currentPage === 'login') {
                var loginData = {
                 ...
                 /**
                $scope.loginDataPromisePost = newHttpPostQuery.postData('http://harristest.com.mocha6001.mochahost.com/foodme/oauth/token', $httpParamSerializer($scope.data), $scope.encoded);
                ...
                */
                ...
                /*some URL redirect part
                var currentPage = window.location.href,
                    trigger = 'index.html#/app/',
                    basicUrl = currentPage.substr(0, currentPage.indexOf(trigger) + trigger.length);
                console.log(basicUrl);
                window.location.href = basicUrl+ 'add_restaurant';
                 */
                ...
                AuthService.registration(registrationData).then(function() {
                    ...
                    if(registered === 'true') {
                        ...
                        $timeout(closeRegisteredWindow, 6000);
                    }else {
    ...
    $scope.fbLogin = function () {
        Facebook.login(function(response) {
            ...
            if (response.status === 'connected') {
                ...
                Facebook.api('/me', function(myResponse) {
